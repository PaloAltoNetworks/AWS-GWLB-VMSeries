...
# 1 VPC
# 1 IGW
# 4 SUBNETS (AGWE, ALB-1, ALB-2, APP)
# ---------------------------------------------------------------------------------------------------------------------
...
resource "aws_subnet" "app_subnet" {
  vpc_id            = aws_vpc.app_vpc.id
  cidr_block        = cidrsubnet(aws_vpc.app_vpc.cidr_block, 3, 0)
...
resource "aws_subnet" "app_agwe_subnet" {
  ...
  availability_zone = var.availability_zone  #data.aws_availability_zones.all.names[tostring(each.key - 1)]
  tags = {
    Name = "app-agwe-subnet-${random_id.deployment_id.hex}"
  }
  depends_on = [aws_vpc.app_vpc]
}
...
resource "aws_subnet" "alb_subnet" {
  ...
  vpc_id            = aws_vpc.app_vpc.id
  cidr_block        = cidrsubnet(aws_vpc.app_vpc.cidr_block, 3, 2+count.index)
...
# 4 ROUTE TABLES (APP, ALB, AGWE, IGW)
# ---------------------------------------------------------------------------------------------------------------------
...
resource "aws_route_table" "agwe-rt" {
  ...
  tags = {
    Name = "app-agwe-rt-${random_id.deployment_id.hex}"
  }
}
